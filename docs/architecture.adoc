...
micro-fe/
├── packages/
│   └── fragment-elements/      # Web Component + SDK compartilhado
│       ├── src/
│       │   ├── fragment-frame.ts       # <fragment-frame> Web Component
...
* `<fragment-frame>` encapsula iframe e comunicação
...
[source,text]
----
App (Angular) → frameSDK → postMessage → <fragment-frame> → CustomEvent → Shell
----
...
* link:../packages/fragment-elements/README.adoc[Fragment Elements Documentation]
* link:../packages/fragment-elements/docs/concepts/architecture.adoc[Detailed Architecture]
* link:../packages/fragment-elements/docs/advanced/function-serialization.adoc[Function Serialization]
