...
**Purpose:** Prevents indefinite waiting if INIT message is not received from the frame.
...
// Create frame with typed props
const frame = document.createElement('z-frame') as Frame;
frame.setAttribute('name', 'user-form');
frame.setAttribute('src', 'http://localhost:3000');

// Type-safe property assignment
const typedFrame = frame as Frame & MyFrameProps;
typedFrame.apiUrl = 'https://api.example.com';
typedFrame.theme = 'dark';
typedFrame.onSave = async (data) => {
...
const unwatch = frameSDK.watch(['theme'], (changes) => {
  if ('theme' in changes && changes.theme) {
    ...
    const typedTheme = newTheme as 'light' | 'dark';
