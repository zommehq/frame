...
graph TB
    ...
    subgraph "Layer 2: Origin Validation"
        ...
        B1[Prevents: Message injection, unauthorized frames]
    end
...
sequenceDiagram
    ...
    participant F as Frame Application
    participant MC as MessageChannel

    P->>MC: Create MessageChannel
    Note over MC: port1 (parent), port2 (frame)
    ...
    alt Origin is valid
        ...
        F->>F: Extract MessagePort from ports[0]
...
// Frame: Validate origin on INIT (one-time only)
...
flowchart TD
    ...
    Q1 -->|No| Q2{Parent or Frame<br/>manages auth?}

    Q2 -->|Parent| Option2[Option 2: Parent-managed Tokens]
    Option2 --> Pro2["✅ Centralized control<br/>✅ Frames stay stateless<br/>⚠️ Parent provides API client<br/>⚠️ Token in memory"]

    Q2 -->|Frame| Option3[Option 3: Frame-managed Tokens]
    Option3 --> Pro3["✅ Frame independence<br/>⚠️ Parent validates requests<br/>⚠️ Token exposed to frame<br/>⚠️ Requires trust"]
...
- [ ] All frames use HTTPS
- [ ] CSP configured for parent and frames
- [ ] `frame-ancestors` set on frame applications
- [ ] Sandbox permissions minimized
- [ ] No secrets passed via attributes
- [ ] All inputs validated
- [ ] Error monitoring in place
- [ ] Dependencies up to date
- [ ] Third-party frames audited
