 ...
 *
 * Shared validation logic used by both SDK and FragmentFrame
 ...
 */
export function isValidMessageStructure(
  message: unknown
...
export function validateMessage(
  ...
  if (!isWhitelistedMessageType(data.type)) {
    console.warn(
      `${logPrefix} Unknown message type (potential attack): ${data.type}`
    );
    return null;
  }
