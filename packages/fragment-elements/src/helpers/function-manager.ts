 ...
 *
 * Used by both FragmentFrame (parent) and FrameSDK (child).
 */
export class FunctionManager {
   ...
   */
  serialize(value: unknown): { serialized: unknown; transferables: Transferable[] } {
   ...
   */
  private async _callRemoteFunction(fnId: string, params: unknown[]): Promise<unknown> {
   ...
   * Must be called when destroying the fragment or parent component.
   */
  cleanup(): void {
    // Clear ALL pending timeouts and reject pending calls
    for (const [callId, pending] of this._pendingFunctionCalls) {
      clearTimeout(pending.timeout);
