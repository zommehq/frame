...
The `FunctionManager` class manages function serialization, remote calls, and lifecycle for bidirectional RPC between parent and fragment.
...
[source,typescript]
----
import { FunctionManager } from '@micro-fe/fragment-elements';
----
...
**CRITICAL:** Must be called when destroying the fragment or parent component.
...
**Integration with Fragment Lifecycle:**

[source,typescript]
----
// In FragmentFrame.disconnectedCallback()
...
// 1. Create manager in FragmentFrame
const manager = new FunctionManager((message, transferables) => {
this._sendToIframe(message, transferables);
});
...
// 3. Parent receives FUNCTION_CALL from fragment
...
----
try {
const result = await fragment.remoteFunction();
} catch (error) {
if (error.message.includes('not found')) {
  console.log('Function not available');
  // Fallback behavior
} else if (error.message.includes('disconnected')) {
  console.log('Fragment disconnected');
  // Handle disconnect
} else {
  // Other error
  console.error('Function call failed:', error);
...
[source,typescript]
----
const result = await fragment.validateData(data);
...
* **Fragment Frame**: See link:../references/fragment-frame.adoc[Fragment Frame] for FragmentFrame integration
