...
/**
 * Props passed from parent to fragment.
 *
 * Can include functions that will be automatically serialized and made callable
 * across the iframe boundary. Fragment can call parent functions via frameSDK.props.functionName().
 *
 * Fragment can expose functions to parent via frameSDK.register() which makes them
 ...
 * };
 *
 * // Fragment accesses props and calls parent functions
 * const theme = frameSDK.props.theme;
 * await frameSDK.props.onSave({ id: 123, name: 'Item' });
 *
 * // Fragment exposes functions to parent
 * frameSDK.register({
 *   refresh: () => loadData(),
 *   export: async (format) => generateExport(format)
 * });
 *
 * // Parent receives and calls fragment functions
 ...
 * @example
 * ```typescript
 * const handler: WatchHandler<FragmentFrameProps> = (changes) => {
 *   if ('theme' in changes) {
