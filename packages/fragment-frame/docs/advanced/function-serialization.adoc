...
Instead, Fragment Frame implements a **Remote Procedure Call (RPC)** system over `postMessage`:
...
Fragment Frame creates a **registry-based proxy system**:
...
4. **Fragment creates proxy**:
   ```typescript
   // Fragment receives token and creates callable proxy
...
When the fragment calls the proxy:
...
1. **Fragment: Serialize arguments**
   ```typescript
   const serializedArgs = structuredClone([{ name: 'John' }]);
   ```

2. **Fragment: Send RPC request via postMessage**
   ```json
   {
...
6. **Fragment: Deserialize result**
   ```typescript
   const result = structuredClone(response.result);
   ```

7. **Fragment: Resolve proxy promise**
   ```typescript
...
// ❌ Fragment does NOT have access to counter directly
...
fragment.onSave = service.save.bind(service);
...
// ❌ Fragment never sees 'apiKey' or UserService class
----
