...
Fragment Frame uses MessageChannel API for dedicated, secure communication between parent and fragment applications.

== MessageChannel vs Window PostMessage

Fragment Frame uses **MessageChannel** instead of `window.postMessage` directly for several advantages:
...
The following diagram shows the complete bidirectional message flow between parent and fragment:
...
sequenceDiagram
    ...
    participant Fragment as Fragment Application

    Note over Parent,Fragment: Initialization Phase
    Parent->>Frame: Create element
    Frame->>IFrame: Create iframe
    Frame->>Frame: Create MessageChannel
    IFrame->>SDK: Load fragment app
    ...
    SDK->>Fragment: Initialize with props

    Note over Parent,Fragment: Ready Phase
    SDK->>Frame: __READY__ (via MessagePort)
    Frame->>Parent: Emit 'ready' event

    Note over Parent,Fragment: Operation Phase
    Parent->>Frame: Update attribute/property
    Frame->>SDK: __ATTRIBUTE_CHANGE__
    SDK->>Fragment: Update props
    SDK->>Fragment: Trigger watch handlers

    Fragment->>SDK: emit('custom-event', data)
    SDK->>Frame: __CUSTOM_EVENT__
    Frame->>Parent: Emit CustomEvent

    Note over Parent,Fragment: Function Calls
    Parent->>Frame: Call fragment.callback()
    Frame->>SDK: __FUNCTION_CALL__
    SDK->>Fragment: Execute function
    Fragment-->>SDK: Return result
    SDK-->>Frame: __FUNCTION_RESPONSE__
    Frame-->>Parent: Resolve promise

    Fragment->>SDK: Call props.onSave()
    SDK->>Frame: __FUNCTION_CALL__
    Frame->>Parent: Execute function
    Parent-->>Frame: Return result
    Frame-->>SDK: __FUNCTION_RESPONSE__
    SDK-->>Fragment: Resolve promise

    Note over Parent,Fragment: Cleanup Phase
    Parent->>Frame: Disconnect element
    Frame->>SDK: __FUNCTION_RELEASE__
...
----
flowchart TD
    A[Parent sends INIT] -->|window.postMessage| B[Fragment receives message]
...
Fragment Frame automatically serializes/deserializes:
...
flowchart LR
    ...
    F --> G[Fragment receives<br/>serialized token]
    G --> H[Deserialize]
    H --> I[Create proxy<br/>function]
    I --> J[Fragment calls<br/>proxy]
    ...
    N --> O[Fragment receives<br/>result]
